...
public struct DiskSparse2D<Coord: BinaryInteger, QVec: BinaryFloatingPoint> {
    ...
    init<InitCoord: BinaryInteger, InitQvec: BinaryFloatingPoint>(c: InitCoord, r: InitCoord, q: [InitQvec]) {
        self.c = Coord(c)
        self.r = Coord(r)
        self.q = q.map({QVec($0)})
    }
...
public struct DiskSparse3D<Coord: BinaryInteger, QVec: BinaryFloatingPoint> {
    ...
    init<InitCoord: BinaryInteger, InitQvec: BinaryFloatingPoint>(i: InitCoord, j: InitCoord, k: InitCoord, q: [InitQvec]) {
        self.i = Coord(i)
        self.j = Coord(j)
        self.k = Coord(k)
        self.q = q.map({QVec($0)})
...
public struct DiskSparse2DAnd3D<Coord: BinaryInteger, QVec: BinaryFloatingPoint> {
    ...
    init<InitCoord: BinaryInteger, InitQvec: BinaryFloatingPoint>(c: InitCoord, r: InitCoord, i: InitCoord, j: InitCoord, k: InitCoord, q: [InitQvec]) {
        self.c = Coord(c)
        self.r = Coord(r)
        self.i = Coord(i)
        self.j = Coord(j)
