...
//                qOutputLength = 4
    ...
    func loadQVecFilesRotation(fromDir dir: String, planeNum: Int) throws -> [[[Float32]]] {
        let dirURL = dataDirURL.appendingPathComponent(dir)

        let reg = qVecRotationBinRegex[planeNum]
        return try loadFilesRotation(fromDir: dirURL, regex: reg)

    }


    func loadF3FilesRotation(fromDir dir: String, planeNum: Int) throws -> [[[Float32]]] {
        let dirURL = dataDirURL.appendingPathComponent(dir)

        let reg = F3RotationBinRegex[planeNum]
        return try loadFilesRotation(fromDir: dirURL, regex: reg)

    }


    func loadFilesRotation(fromDir dirURL: URL, regex: String) throws -> [[[Float32]]] {
        //https://stackoverflow.com/questions/27721418/getting-list-of-files-in-documents-folder/27722526
        ...
        print(dirURL, regex, filteredBinFileNames)
        return try loadAndAllocateGRID(dirURL: dirURL, fileNames: filteredBinFileNames)
    ...
    fileprivate func loadAndAllocateGRID(dirURL: URL, fileNames: [String]) throws -> [[[Float32]]] {
...
//                qOutputLength = 4
            ...
            let lenBytesGrid = 2
            let lenBytesQ = 4
            let lenBytesStruct = lenBytesGrid * 3 + lenBytesRowCol * 2 + qOutputLength * lenBytesQ
            ...
            data.withUnsafeBytes{ (ptr: UnsafeRawBufferPointer) in
                ...
                for i in stride(from: 0, to: dim.binFileSizeInStructs * lenBytesStruct, by: lenBytesStruct)  {
                    ...
                    let _ = Int(ptr.load(fromByteOffset: i, as: UInt16.self))
                    let _ = Int(ptr.load(fromByteOffset: i + lenBytesGrid, as: UInt16.self))
                    let _ = Int(ptr.load(fromByteOffset: i + lenBytesGrid, as: UInt16.self))


                    let col = Int(ptr.load(fromByteOffset: i + lenBytesGrid, as: UInt16.self))
                    let row = Int(ptr.load(fromByteOffset: i + lenBytesRowCol, as: UInt16.self))

                    for q in 0..<qOutputLength {
                        plane[row][col][q] = ptr.load(fromByteOffset: i + lenBytesGrid * 3 + lenBytesRowCol * 2 + (q * lenBytesQ), as: Float32.self)
                    }
